...
def post_new():
    ...
    :param redirect: (query) whether to redirect (code 303) to the generated url (1, the default) or to return the generated url as text (0)
    ...
    if strtobool(bt.request.query.redirect or '1'):
    	bt.redirect(f'/{snippet.id}.{ext}')
    else:
        bt.response.content_type = 'text/plain'
        return f"/{snippet.id}.{ext}"
